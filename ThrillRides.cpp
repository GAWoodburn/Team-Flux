...
ThrillRide::ThrillRide(int& playerMoney, bool& playerDeath) {
	while (playerDeath == false)
	{
		...
		{
			cout << "\nYour options are ('A', 'B', 'C', 'D', 'E', or 'F')!";  // Throw error if not valid
		...
		case 'b':
			...
			{
				cout << "\nWelcome in, you've made a wise decision...";
		...
		case 'd':
			...
			else if (choice == 'Y' || choice == 'y')  // Yell
			{
		...
		case 'E':  // Test Track
		case 'e':
			playerMoney = playerMoney - TESTTRACK;
			...
			else if (choice == 'L' || choice == 'l')  // Lamborghini
			{
		...
		while (playerMoney <= 0)  // Check for playerMoney below or at 0
		{
			...
			playerDeath = true;   // Set playerDeath true
			playerMoney = 0;	  // Set playerMoney to 0
			goto leave;
		}
...
void ThrillRide::menu(char& decision, string& delimiter, int&playerMoney)  // Menu function that accepts char decision and delimiter string
{
